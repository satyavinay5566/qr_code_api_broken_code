...
def authenticate_user(username: str, password: str):
    """
    Authenticates a user against environment-configured credentials.
    """
    if username == ADMIN_USER and password == ADMIN_PASSWORD:
        return {"username": username}
    logging.warning(f"Authentication failed for user: {username}")
...
def encode_url_to_filename(url):
    """
    Encodes a URL into a base64-safe string for filenames.
    """
...
def decode_filename_to_url(encoded_str: str) -> str:
    """
    Decodes a base64-encoded string back into the original URL.
    """
    padding_needed = 4 - (len(encoded_str) % 4)
    if padding_needed != 4:
        encoded_str += "=" * padding_needed
